...
class FusionBuilder {
	...
	public function init() {
		...
		add_filter( 'pre_do_shortcode_tag', [ $this, 'add_render_logic_check_pre_do_shortcode' ], 10, 4 );
	}
	...
	public function filter_post_content( $post_content, $handle_slashes = true, $force_check = false ) {
		if ( apply_filters( 'awb_is_unfiltered_html_allowed', current_user_can( 'unfiltered_html' ) ) && ! $force_check ) {
			return $post_content;
		}
	...
	public function get_extra_google_fonts() {
		if ( null === $this->extra_fonts ) {
			$id           = get_query_var( 'fb-edit' ) ? get_query_var( 'fb-edit' ) : fusion_library()->get_page_id();
	...
	public static function activation() {
		...
		if ( function_exists( 'fusion_builder_auto_activate_element' ) ) {
			...
			if ( version_compare( $db_version, '3.13.0', '<' ) ) {
				fusion_builder_auto_activate_element( 'awb_dd' );
				fusion_builder_auto_activate_element( 'awb_text_path' );
			}

			if ( version_compare( $db_version, '3.13.2', '<' ) ) {
				fusion_builder_auto_activate_element( 'fusion_form_turnstile' );
			}
		}
	...
	public function init_shortcodes() {
		...
		require_once FUSION_BUILDER_PLUGIN_DIR . 'shortcodes/awb-dd.php';
		require_once FUSION_BUILDER_PLUGIN_DIR . 'shortcodes/awb-text-path.php';
	...
	private function includes() {
		...
		require_once FUSION_BUILDER_PLUGIN_DIR . 'inc/class-fusion-form-auth-actions.php';
	...
	public function add_element_media_query_styles() {
		...
		$small_css .= 'body .sm-text-align-justify{text-align:justify !important;}';
		...
		$medium_css .= 'body .md-text-align-justify{text-align:justify !important;}';
		...
		$large_css .= 'body .lg-text-align-justify{text-align:justify !important;}';
		...
		$absolute_css = 'position:absolute;width:100%;';
	...
	public static function set_element_description_map() {
		...
		$element_option_map['current_page_color']['fusion_breadcrumbs']  = [
			'theme-option' => 'breadcrumbs_current_page_color',
			'reset'        => true,
		];		
		$element_option_map['prefix_color']['fusion_breadcrumbs']  = [
			'theme-option' => 'breadcrumbs_prefix_color',
			'reset'        => true,
		];
		$element_option_map['separator_color']['fusion_breadcrumbs']  = [
			'theme-option' => 'breadcrumbs_separator_color',
			'reset'        => true,
		];
		...
		// Hotspot element.
		$element_option_map['popover_heading_background_color']['fusion_image_hotspots']  = [
			'theme-option' => 'popover_heading_bg_color',
			'reset'        => true,
		];
		$element_option_map['popover_content_background_color']['fusion_image_hotspots']  = [
			'theme-option' => 'popover_content_bg_color',
			'reset'        => true,
		];
		$element_option_map['popover_border_color']['fusion_image_hotspots']  = [
			'theme-option' => 'popover_border_color',
			'reset'        => true,
		];
		$element_option_map['popover_text_color']['fusion_image_hotspots']  = [
		...
		$element_option_map['autoplay_speed']['fusion_post_cards']      = [
			'theme-option' => 'carousel_speed',
			'type'         => 'range',
		];
		
		// Post Card 
		$element_option_map['button_layout']['fusion_post_card_image']             = [
			'theme-option' => 'woocommerce_product_box_design',
			'type'         => 'select',
		];
		...
		$element_option_map['autoplay_speed']['fusion_tb_post_card_archives']      = [
			'theme-option' => 'carousel_speed',
			'type'         => 'range',
		];
		...
		$element_option_map['fully_filled_color']['fusion_progress']       = [
			'theme-option' => 'progressbar_fully_filled_color',
			'reset'        => true,
		];
		...
		// User Login Element.
		$element_option_map['heading_color']['fusion_login'] = [
			'theme-option' => 'h3_typography',
			'subset'       => 'color',
			'reset'        => true,
		];		
		$element_option_map['caption_color']['fusion_login'] = [
			'theme-option' => 'body_typography',
			'subset'       => 'color',
			'reset'        => true,
		];
		...
		$element_option_map['heading_color']['fusion_register'] = [
			'theme-option' => 'h3_typography',
			'subset'       => 'color',
			'reset'        => true,
		];		
		$element_option_map['caption_color']['fusion_register'] = [
			'theme-option' => 'body_typography',
			'subset'       => 'color',
			'reset'        => true,
		];
		...
		$element_option_map['heading_color']['fusion_lost_password'] = [
			'theme-option' => 'h3_typography',
			'subset'       => 'color',
			'reset'        => true,
		];		
		$element_option_map['caption_color']['fusion_lost_password'] = [
			'theme-option' => 'body_typography',
			'subset'       => 'color',
			'reset'        => true,
		];
		...
		$element_option_map['autoplay_speed']['fusion_images']      = [
			'theme-option' => 'carousel_speed',
			'type'         => 'range',
		];
		...
		// Turnstile field.
		$element_option_map['appearance']['fusion_form_turnstile'] = [
			'theme-option' => 'turnstile_appearance',
			'type'         => 'select',
		];
		$element_option_map['theme']['fusion_form_turnstile'] = [
			'theme-option' => 'turnstile_theme',
			'type'         => 'select',
		];
		$element_option_map['size']['fusion_form_turnstile'] = [
			'theme-option' => 'turnstile_size',
			'type'         => 'select',
		];
		$element_option_map['language']['fusion_form_turnstile'] = [
			'theme-option' => 'turnstile_language',
			'type'         => 'select',
		];
	...
	public function add_render_logic_check_pre_do_shortcode( $output, $tag, $atts, $m ) {
		...
		if ( ( false !== strpos( $tag, 'fusion_' ) || false !== strpos( $tag, 'awb_' ) ) && ! Fusion_Builder_Conditional_Render_Helper::should_render( $atts, $content ) ) {
			if ( 'fusion_container' === $tag ) {
				global $global_container_count;
